...
{     
    ...
    $tmp_name = $_FILES['my_image']['tmp_name'];
     ...
     if($error === 0)
     {
      ...
      `calories` = '$calo',`proteins` = '$prot' , `fats` = '$fat',`carbs`= '$carbo', `category`= '$cate', `image` = '$imageandpath',
       `course`='$course' where `recipe_id` = '222' ";
 ...
 <!------------------display data--------------->
 <?php 
    $sql= "select * from `dietian_recipies` where `recipe_id` = '222'";
 ...
 ?>
    ...
    <form action =" " method="POST" enctype="multipart/form-data">
      <!------------------------------------------------DASHBOARD--------------------------------------------------------->
       ...
       <button type="submit" name="editbtn" id="edit" style="height:50px; width:140px;background-color:#D257E6;color:white; border-radius:15px;"> Edit </button>
