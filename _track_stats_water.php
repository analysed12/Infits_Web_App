...
if(isset($_POST['savegoal'])){
    ...
    $query="UPDATE watertrackerdt SET goal = $goal WHERE clientID= '$clientId' AND 
...
// All Data Total Sum
$allDataSum = fetchDataSql($clientId, '', '', '', $today->format('d'), $today->format('m'), $today->format('y'), 1)[0]['SUM(drinkConsumed)'];
// Today Data Sum
$todayData = fetchDataSql($clientId, $today->format('d'), $today->format('m'), $today->format('y'), $today->format('d'), $today->format('m'), $today->format('y'),2)[0]['SUM(drinkConsumed)'];
// Week Average
$pastWeek =new DateTime();
$pastWeek->modify('-1 week');
$weekAvg = fetchDataSql($clientId,$pastWeek->format('d'), $pastWeek->format('m'), $pastWeek->format('y'), $today->format('d'), $today->format('m'), $today->format('y'))[0]['avg(drinkConsumed)'];
// Month Average
$pastMonth = new DateTime();
$pastMonth->modify('-1 month');
$monthAvg = fetchDataSql($clientId,$pastMonth->format('d'), $pastMonth->format('m'), $pastMonth->format('y'), $today->format('d'), $today->format('m'), $today->format('y'))[0]['avg(drinkConsumed)'];
?>
...
?>
                    <div class="activity-box">
                        ...
                        <div class="activity-data">
                            ...
                            <span class="down"><?php echo ($pastActivityData[$j-1]['drinkConsumed']) ?> Litres</span>
                        </div>
...
$calorieConsumed = fetchDataSql($clientId, $today->format('d'), $today->format('m'), $today->format('y'), $today->format('d'), $today->format('m'), $today->format('y'), 2)[0]['SUM(drinkConsumed)'];
            ...
            <div class="progress-bar-container">
                <div class="flex_box">
                    <div class="total-consumed">
                        <span><?php echo ($calorieConsumed) ?> Litres</span>
                        <p>Consumed</p>
                    </div>
                    <div class="total-remaining">
                        <span><?php echo($calorieRemaining) ?> Litres</span>
                        <p>Remaining</p>
                    </div>
                </div>
...
<script>
    ...
    progressPercent.style.setProperty("background", "conic-gradient( #ADD8E6 <?php echo(100 - $progressPercent) ?>% ,#63AEFF  0)");
</script>
    </div>
</div>
<?php
...
for ($i = 12; $i > 0;$i--){
    ...
    $yearly_Data = (int) fetchDataSql($clientId, $yearly_Month_1->format('d'), $yearly_Month_1->format('m'), $yearly_Month_1->format('y'), $yearly_Month_2->format('d'), $yearly_Month_2->format('m'), $yearly_Month_2->format('y'),3)[0]['avg(drinkConsumed)'];
...
while ($today >= $monthly_Month) {
    $monthly_Data = (int) fetchDataSql($clientId,$monthly_Month->format('d'),$monthly_Month->format('m'),$monthly_Month->format('y'), $monthly_Month->format('d'),$monthly_Month->format('m'),$monthly_Month->format('y'))[0]['avg(drinkConsumed)'];
...
for($i=7;$i>=0;$i--){
    ...
    $weekly_Data = fetchDataSql($clientId, $weekly_Day->format('d'), $weekly_Day->format('m'), $weekly_Day->format('y'), $weekly_Day->format('d'), $weekly_Day->format('m'), $weekly_Day->format('y'))[0]['avg(drinkConsumed)'];
